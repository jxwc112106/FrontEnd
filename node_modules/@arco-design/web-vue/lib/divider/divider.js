...
var _Divider = vue.defineComponent({
  ...
  setup(props, {
    ...
    const mergedStyles = vue.computed(() => {
      const styles = {};
      if (props.size) {
        styles[isHorizontal.value ? "border-left-width" : "border-bottom-width"] = is.isNumber(props.size) ? `${props.size}px` : props.size;
      }
      if (props.type) {
        styles[isHorizontal.value ? "border-left-style" : "border-bottom-style"] = props.type;
      }
      if (props.margin) {
